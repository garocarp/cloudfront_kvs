...
export class FunctionWithStore extends Function {
  ...
  constructor(scope: Construct, id: string, props: FunctionPropsWithStore) {
    ...
    const keyValueStore = new KeyValueStore(scope, `${id}-store`, {
      source: ImportSource.fromAsset(keyValueStoreImportSourcePath),
    });

    const functionCodeRaw = readFileSync(entryPath).toString();
    const functionCodeWithKvsId = functionCodeRaw.replace(
      "__KVS_ID__",
    ...
    // Store the keyValueStoreId as an instance property
    this.keyValueStoreArn = keyValueStore.keyValueStoreArn;
  }

  // Method to retrieve keyValueStoreId
  public getKeyValueStoreArn(): string {
